    ...
    while (++depth <= MAX_PLY && !Signals.stop && (!Limits.depth || depth <= Limits.depth))
    {
        ...
        if (Limits.use_time_management() && !Signals.stop && !Signals.stopOnPonderhit)
        {
			...
			if (multiPV==2)
			{
				...
				{
					multiPV=1;
					diff=diff/4-Value(5);//diff is changed to 0 if the program changes its mind later and if it does not change its mind later we want to use the last known difference for better time management 
				}
			}
